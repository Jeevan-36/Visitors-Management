...
const ManagerReports = () => {
  ...
  const dummyFlats = [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115];
const statusConfig = {
  Approved: {
    ...
    <div className="bg-gray-900 min-h-screen text-white p-6 font-sans">
      ...
      <div className="bg-gray-800 rounded-lg p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
          <div>
            <label htmlFor="phoneNo" className="block text-sm font-medium text-gray-300 mb-1">Phone Number</label>
            <input 
              type="tel" 
              id="phoneNo" 
              value={phoneNo} 
          ...
          <div>
            ...
            >
              ...
              {dummyFlats.map((flat) => (<option key={flat} value={flat}>{flat}</option>))}
            </select>
          </div>
          <div>
  ...
  <h2 className="text-xl font-semibold text-gray-300 mb-4">Search Results</h2>
  <div className="overflow-x-auto">
    <table className="min-w-full text-sm text-left text-gray-400">
      ...
      <tbody>
        {loading ? (
          <tr>
            <td colSpan="7" className="text-center py-4">Loading results...</td>
          </tr>
        ) : visits.length > 0 ? (
          visits.map((visit) => (
            <tr key={visit.id} className="border-b border-gray-700 hover:bg-gray-700">
              <td className="py-4 px-6 font-medium text-white whitespace-nowrap">{visit.visitor.name}</td>
              ...
              <td className="py-4 px-6">
                ...
                <span className={`inline-flex items-center py-1 px-2.5 rounded-full text-xs font-semibold ${
                  statusConfig[visit.status]?.styles || statusConfig.Pending.styles
                }`}>
                  {visit.status}
                </span>
              </td>
          ...
          <tr>
            <td colSpan="7" className="text-center py-4 text-gray-500">
              No results to display. Please apply filters and click "Search".
            </td>
          </tr>
        )}
      </tbody>
